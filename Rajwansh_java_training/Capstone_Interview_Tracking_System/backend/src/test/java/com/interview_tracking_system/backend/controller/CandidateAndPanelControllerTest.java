...
class CandidateAndPanelControllerTest {
    ...
    void candidateControllerMethodsShouldDelegate() {
        ...
        assertEquals(
                ...
                candidateController.applyToJob(
                        profile,
                        new MockMultipartFile("resumeFile", "r.pdf", "application/pdf", "x".getBytes()),
    ...
    void panelControllerMethodsShouldDelegate() {
        ...
        UsernamePasswordAuthenticationToken principal = new UsernamePasswordAuthenticationToken("panel@test.com", "x");

        assertEquals(
                10L,
                panelController.getPanelInterviews(principal).get(0).getInterviewId());
    }
}
